...
export default function Sidebar({ role }) {
  ...
  return (
    <aside className="hidden lg:flex lg:flex-col w-64 bg-gradient-to-b from-slate-900 via-slate-900 to-[#070b12] text-white min-h-screen border-r border-white/5 relative overflow-hidden">
      {/* Dynamic colored background light-spot for premium biological feel */}
      ...
      <div className="p-6 border-b border-white/10 relative z-10">
        <div className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-primary to-primary-light flex items-center justify-center shadow-lg shadow-primary/20 group-hover:scale-105 transition-transform duration-300">
            <svg className="w-5 h-5 text-white animate-pulse-gentle" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      <nav className="flex-1 p-4 space-y-1 relative z-10 mt-4">
        {items.map(item => (
          <NavLink
            ...
            className={({ isActive }) => `sidebar-link flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold tracking-wide relative overflow-hidden transition-all duration-300 group ${
              isActive 
                ? 'active bg-white/10 text-white shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] border-l-4 border-primary' 
