  ...
  useEffect(() => {
    if (user?.role === 'Doctor') {
      const fetchCode = async () => {
        try {
          const { data } = await api.get('/doctor/my-code')
          setDoctorCode(data.code)
        } catch {
          const lastName = user.name.split(' ').pop().toUpperCase()
          const shortId = (user.id || 'd-uuid').slice(0, 4).toUpperCase()
          setDoctorCode(`DR-${lastName}-${shortId}`)
        }
      }
      fetchCode()
    }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-80 border-r border-surface-border bg-surface-card flex flex-col">
            {user?.role === 'Doctor' && (
              <div className="p-4 bg-primary-50/50 dark:bg-primary-100/5 border-b border-surface-border select-none">
                <p className="text-[9px] font-extrabold text-primary uppercase tracking-wider mb-1">
                  Your Clinical Invite Code
                </p>
                <div 
                  onClick={handleCopyCode}
                  className="flex items-center justify-between bg-surface-card border border-primary-100/60 dark:border-primary-100/10 rounded-xl px-3 py-2 cursor-pointer hover:bg-primary-50 dark:hover:bg-primary-100/10 transition-colors group"
                  title="Click to copy invite code"
                >
                  <code className="text-xs font-mono font-bold text-text-heading">
                    {doctorCode || 'Generating...'}
                  </code>
                  <span className="text-[9px] font-bold text-primary group-hover:scale-105 transition-transform flex items-center gap-1">
                    {copied ? 'Copied!' : (
                      <>
