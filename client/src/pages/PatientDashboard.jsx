...
export default function PatientDashboard() {
  ...
  const handleLinkDoctor = async (e) => {
    ...
    setLinkError('')
    try {
      const { data } = await api.post('/doctor/link-code', { code: linkCode })
      if (data.success) {
        setLinkCode('')
        await fetchData()
      }
    } catch (err) {
      setLinkError(err.response?.data?.error || 'Failed to link doctor. Please check code.')
    } finally {
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8">
          ...
          <div id="readings-trends-section" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-surface-card rounded-2xl border border-surface-border p-6 shadow-soft">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-text-secondary text-xs mt-0.5">Fasting & post-meal historic mappings</p>
                </div>
                  ...
                  {['weekly', 'monthly'].map(f => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${
                        filter === f ? 'bg-surface-elevated text-text-heading' : 'text-text-muted hover:text-text-secondary'
                      }`}
                    >
                      {f}
                    </button>
            ...
            <div className="bg-surface-card rounded-2xl border border-surface-border p-6 shadow-soft">
              ...
              <p className="text-text-secondary text-xs mb-6">Proportion of safe vs danger readings</p>
            ...
            <div id="chat-consultation-section" className="bg-surface-card rounded-2xl border border-surface-border p-6 shadow-soft flex flex-col h-[340px] lg:h-auto justify-between transition-all duration-300">
              ...
              {!linkedDoctor ? (
                <div className="flex-1 flex flex-col justify-center items-center py-6 text-center">
                  <div className="w-12 h-12 rounded-full bg-primary-50 dark:bg-primary-100/10 text-primary flex items-center justify-center mb-3">
                  ...
                  <h4 className="text-xs font-bold text-text-heading mb-1.5">No Doctor Connected</h4>
                  <p className="text-[10px] text-text-secondary max-w-[220px] leading-relaxed mb-4">
                    Enter your doctor's clinical invite code to unlock advice, consults, and live monitoring.
                  </p>
                  <form onSubmit={handleLinkDoctor} className="w-full space-y-2">
                    <input
                      type="text"
                      required
                      placeholder="e.g. DR-JENKINS-D-UU"
                      value={linkCode}
                      onChange={e => setLinkCode(e.target.value)}
                      className="w-full px-3 py-2 text-center text-xs font-mono font-semibold rounded-lg border border-slate-200 dark:border-slate-700 focus:outline-none focus:border-primary bg-surface text-text-body placeholder-text-muted"
                    />
                    {linkError && <p className="text-[9px] text-rose-500 font-bold">{linkError}</p>}
                    <button
                      type="submit"
                      disabled={linkLoading}
                      className="w-full py-2 bg-primary hover:bg-primary-dark text-white rounded-lg font-bold text-xs transition-colors cursor-pointer disabled:opacity-50"
                    >
                      {linkLoading ? 'Linking...' : 'Link Care Team'}
                    </button>
                  </form>
                </div>
              ) : (
